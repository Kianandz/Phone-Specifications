...
import Alert from "../../Fragment/Alert";
...
const UpdatePhone = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    // Anda perlu mengganti 'PHONE_ID' dengan ID telepon yang akan diupdate

    try {
      const response = await fetch(`${API_BASE_URL}/updatePhone/${id}`, {
  ...
  useEffect(() => {
    document.title = `Update Smartphone ID : ${id}`;
  });
  return (
    <>
      {showAlertError && (
        <Alert
          ...
          position="fixed top-0 right-4 z-50"
          padding="px-6 py-4 rounded-lg"
        />
      )}
      <div className="relative fade-in p-8">
        {isLoggedIn ? (
          <>
            <div className="w-full relative flex items-center justify-between">
              <Title
                className="font-bold text-2xl"
                Title={`Update Data with ID : ${id}`}
              />
              <Text
                className="text-base px-6 py-2 bg-gray-400 hover:bg-gray-600 rounded-lg transition-all cursor-pointer"
                Text="Cancel"
                onClick={prevPage}
              />
            </div>
